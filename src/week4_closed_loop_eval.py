...
Closed-loop evaluation script for Week 4 VLN project.
"""
...
def evaluate_episode(pred_traj, gt_traj, goal_pos):
    ...
    goal_dist = math.sqrt(
        (goal_x - last_x)**2 + (goal_y - last_y)**2
    )
    sr = 1 if goal_dist <= 3.0 else 0

    osr = 0
    for x, y in filtered:
        if math.sqrt((goal_x - x)**2 + (goal_y - y)**2) <= 3.0:
            osr = 1
            break
...
def load_model(adapter_path):
    ...
    print(f"Loading processor from base model...", flush=True)
    processor = AutoProcessor.from_pretrained(
        "/home/ad10/2026_deeplearning/deep26_week4/models/Qwen3-VL-2B-Instruct",
    ...
    messages = [
        {
            ...
            "content": [{"type": "text", "text": (
                ...
                "Given the full navigation instruction and two egocentric "
                "images, predict the next action.\n"
...
def load_episode_data(eval_json_path, fine_val_gz, fine_train_gz):
    ...
    print(f"Total episodes in both splits: {len(all_episodes)}")
    ...
    for ep in raw_episodes:
        ...
        if ep_data is None:
            print(f"WARNING: Episode {episode_id} not found in any split")
            continue
...
# ── IsaacSim episode runner ──────────────────────────────────────
def run_episode_in_isaac(
    model, processor, episode, scene_dir, work_dir,
    timeout_steps=400, headless=True
):
    import sys
    ...
    robot_yaw = 2 * math.asin(
        max(-1.0, min(1.0, float(start_rot[3])))
    )
    print("start_pos:", start_pos)
    print("start_rot:", start_rot)
    print("robot_yaw_deg:", math.degrees(robot_yaw))

    pred_traj = [[robot_pos[0], robot_pos[1]]]
    invalid_count = 0
    step = 0
    ...
    def update_camera():
        cam_x = robot_pos[0] - 0.5 
        cam_y = robot_pos[1] - 0.5 
        cam_z = robot_pos[2] + 0.4
        xformable = UsdGeom.Xformable(
            stage.GetPrimAtPath(camera_path)
        )
        xformable.ClearXformOpOrder()
        xformable.AddTranslateOp().Set(
            Gf.Vec3d(float(cam_x), float(cam_y), float(cam_z))
        )
        look_x = math.sin(-robot_yaw)
        look_y = math.cos(-robot_yaw)
        pitch = math.degrees(math.atan2(
            0.35, math.sqrt(look_x**2 + look_y**2)
        ))
        yaw_deg = math.degrees(math.atan2(look_x, look_y))
        xformable.AddRotateXYZOp().Set(
            Gf.Vec3f(float(pitch), 0.0, float(yaw_deg))
        )
    ...
    def execute_action(action):
        ...
        update_camera()
        pred_traj.append([robot_pos[0], robot_pos[1]])
    ...
    while step < timeout_steps and not done:
        if prev_image is not None and curr_image is not None:
            action, is_invalid = predict_action(
                model, processor, instruction,
                prev_image, curr_image
            )
        else:
        ...
        # Save egocentric frame with action label
        if curr_image is not None:
            ...
            # Draw instruction (top)
            draw.rectangle([0, 0, 512, 50], fill=(0, 0, 0, 180))
            draw.text((8, 8), "TASK: " + instruction[:60], fill=(255, 255, 0))
            # Draw action (bottom)
            draw.rectangle([0, 462, 512, 512], fill=(0, 0, 0, 180))
            draw.text((8, 468), "Step %03d | %s" % (step, action), fill=(0, 255, 0))
            annotated.save(os.path.join(frame_dir, "step_%04d.png" % step))
    ...
    metrics["invalid_rate"] = round(
        invalid_count / max(step, 1), 4
    )

    print(f"\nResults for {episode['episode_id']}:")
    print(f"  SR={metrics['SR']} OSR={metrics['OSR']} "
...
def main():
    ...
    parser.add_argument(
        "--work_dir",
        type=str,
        default="results/closed_loop_eval",
    )
    ...
    # NEW: run only one episode if needed
    parser.add_argument(
        "--episode_idx",
        type=int,
        default=None,
        help="Run only one episode by index. Example: --episode_idx 0",
    )
    ...
    for i, episode in enumerate(episodes):
        ...
        if not os.path.exists(scene_path):
            print(
                f"WARNING: Scene not found: {scene_path}, skipping...",
                flush=True,
            )
            continue

        print(
            f"\n[{i+1}/{len(episodes)}] Running: {episode['episode_id']}",
            flush=True,
        )
